...
//ref link:https://www.youtube.com/watch?v=onw_BtmqSx0&list=PLRwVmtr-pp07QlmssL4igw1rnrttJXerL&index=38
...
class Cow : IComparable<Cow>   // -- Sort, IComparable -- class prop 
{
...
class MyCowComparer : IComparer<Cow>    // -- Sort, IComparer -- (ExternalComparer)alternatives, if IComparable(CompareTo) is not available, unaccessable, or class cant be compare, etc
{
    public int Compare(Cow left, Cow right) // int return negative(leftLESSTHANright), int return 0(leftIsEQUALright),int return positive(leftGREATERTHANright)
    {
        return left.Weight - right.Weight;      // order by weight 
    }
}
...
class MainClass
{
    ...
    static void Main()
    {
        ...
        meCows.Sort(new MyCowComparer());   // order by weight with name
        ...
        //    new Cow("Bacon"), new Cow("Beef"),
        //};
        //meCows.Sort();    // order by name with weight
        //meCows.ForEach(cow => Console.WriteLine(cow.Name));
        //meCows.ForEach(cow => Console.WriteLine(cow.Weight));
        //meCows.ForEach(cow => Console.WriteLine(cow.Name + ": " + cow.Weight));
        //meCows.ForEach(Console.WriteLine);
